...
export function InputIcon({ ...props }: InputIconProps) {
  return (
    <span
      className="group-focus-within:text-gray-100 text-gray-400 group-[&:not(:has(input:placeholder-shown))]:text-gray-100 group-data-[error=true]:text-danger"
      {...props}
    />
  )
}
...
export function Input({ error = false, ...props }: InputProps) {
    return(
        <div 
            ...
            className='group bg-gray-800 h-12 border border-gray-600 rounded-xl px-4 flex items-center gap-2 focus-within:border-gray-100 data-[error=true]:border-danger'
        >
            <span className='group-focus-within:text-gray-100 text-gray-400 group-[&:not(:has(input:placeholder-shown))]:text-gray-100 group-data-[error=true]:text-danger'>
                <Mail/>
            </span>
